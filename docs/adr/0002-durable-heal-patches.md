...
Healed source files are copied to `~/.local/state/issue-dinner/heals/<patch-id>/` with a manifest (id, issueKey, reason, file contents). On each serve start, durable heals sync into the resolved issue-dinner package root before courses run, so healing survives process restarts and npm reinstalls of the binary package.
