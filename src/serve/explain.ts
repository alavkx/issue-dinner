...
export function explainCourseFailure(
  issueKey: string,
  epic: string,
  ...
  if (!rec) {
    return {
      ...
      whatToDo: [`issue-dinner ${epic} cook ${issueKey}`],
    };
  }
  ...
  if (
    ...
    return {
      ...
      whatToDo: [
        ...
        `After the course finishes, run full tests: issue-dinner verify ${issueKey}`,
      ],
    };
  }
  ...
  if (
    ...
    return {
      whatWentWrong: `Git still had uncommitted changes when dinner tried to switch to the ${issueKey} branch.`,
      whatToDo: [
        `Re-run: issue-dinner ${epic} serve --only ${issueKey} (dinner will try to commit or stash automatically)`,
        `Or manually: cd fileservice2 && git status, then commit or stash, then re-run serve`,
        `If work was stashed: git checkout alavoie/cpd-635/${issueKey.toLowerCase()} && git stash list`,
      ],
    };
  }
  ...
  if (rec.status === "agent_complete" || err.includes("Verify failed")) {
    ...
    return {
      ...
      whatToDo: [
        ...
        `Or re-run the story: issue-dinner ${epic} cook ${issueKey} --force`,
      ],
    };
  }
  ...
  if (steps.some((s) => s.includes("Recovery handoff not acceptable"))) {
    return {
      whatWentWrong: `Dinner tried to fix a git/stack problem automatically, but the recovery agent did not confirm success.`,
      whatToDo: [
  ...
  if (
    ...
    return {
      ...
      whatToDo: [
        `Re-run: issue-dinner ${epic} serve --only ${issueKey}`,
        `If it keeps timing out, run: issue-dinner ${epic} cook ${issueKey} --force`,
      ],
    };
  }
  ...
  if (steps.some((s) => s.includes("Recovery run error"))) {
    return {
      ...
      whatToDo: [
        `issue-dinner ${epic} cook ${issueKey} --force`,
  ...
  if (rec.status === "error" && err) {
    return {
      whatWentWrong: err,
      whatToDo: [`issue-dinner ${epic} cook ${issueKey} --force`],
    };
  }
  ...
  return {
    ...
    whatToDo: [`issue-dinner ${epic} status --verbose`, `issue-dinner ${epic} cook ${issueKey} --force`],
  };
}
...
export function explainPreflightFailure(
  ...
  if (
    (message.includes("no commands configured") ||
      message.includes("no test recipe") ||
      message.includes("only slow tests")) &&
    issueKey
  ) {
    return {
      summary: `${issueKey} has no quick test gate for serve — dinner will skip fast checks and continue (integration tests run separately).`,
      steps: [
  ...
  if (message.includes("working tree clean") && fix?.includes("stash")) {
    return {
      summary: "The repo has uncommitted changes before dinner starts.",
      steps: [
        "Dinner will usually commit or stash these automatically on retry",
