...
/**
 * Enforce stacked story order: every prior story must be verified (or skipped this run)
 * before starting the next. Jira `blockedBy` alone is not enough for Graphite stacks.
 */
export const storyOrderBlocks = (
  stories: JiraIssue[],
  index: number,
  ...
  Effect.gen(function* () {
    ...
    for (let i = 0; i < index; i++) {
      ...
      return {
        ok: false,
        blockerKey: prev.key,
        reason: `Prior story ${prev.key} is ${st} — verify or fix before ${current.key}`,
      };
    }
    return { ok: true };
  });
