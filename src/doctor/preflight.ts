...
const recoveryIssueForWorkspace = (
  ...
  Effect.gen(function* () {
    ...
    for (const issue of epicStories) {
      if (
        touches(issue) &&
    ...
    return epicStories.find(touches);
  });
...
> =>
  Effect.gen(function* () {
    ...
    >) {
      ...
      if (!clean) {
        ...
        push(
          issues,
          true,
          `${key}: working tree has uncommitted changes (the orchestrator will recover at stack prep)`,
        );
        continue;
      }
    ...
    if (options.validateVerify !== false) {
      ...
      for (const issue of options.epicStories) {
        if (yield* shouldSkipVerifyPathsForIssue(issue.key)) {
          ...
          push(
            issues,
            true,
            `${issue.key}: verify paths skipped (${status} in run state)`,
          );
          continue;
        }
