...
export function buildAgentPrompt(ctx: PromptContext): string {
  const { issue, roots, config, verifyCommands, selfHeal, toolRoot } = ctx;
  ...
  const sdkLinkSection =
    config.sdkClientPackage &&
    roots.keys.includes("frontend") &&
    roots.keys.includes("sdk")
      ? `
When **frontend** and **sdk** are both in the workspace, issue-dinner links \`${config.sdkClientPackage}\` to the local SDK checkout before you start — use the linked SDK types; do not revert to a published npm version mid-slice.
`
      : "";

  const selfHealSection =
    selfHeal && toolRoot
      ? `
## issue-dinner self-heal (on by default)

If you discover a bug in **issue-dinner itself** while working this story, fix it **inline** before finishing.

- **issue-dinner root:** \`${toolRoot}\` (included in your workspace \`cwd\`)
...
- After you edit issue-dinner source, issue-dinner runs typecheck/build validation and restarts so the fix loads; your story resumes with a clean message.
...
**Inner loop (blocks the serve loop):** fast commands — unit tests, typecheck. Run these locally while building.

**Outer loop (CI / manual):** integration/e2e commands — issue-dinner does not block the serve loop on these when \`serveVerifyGate\` is \`inner\` (default).
