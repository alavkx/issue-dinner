...
import { buildPostHealStoryResumePrompt } from "../self-heal/heal-prompt.js";
import {
  attemptSelfHealFromStory,
  type ServeHealContext,
} from "../self-heal/heal-agent.js";
import { attemptInlineHealFromStory } from "../self-heal/inline-heal.js";
...
> =>
  Effect.gen(function* () {
    ...
    const transcript = options.epic
      ...
      : undefined;
    if (transcript) {
      yield* appendTranscriptLine(transcript, `Story start: ${issue.summary}`);
    }

    const toolBaseline =
      options.selfHeal && options.toolRoot
        ? yield* snapshotSrcFiles(options.toolRoot)
        : undefined;
    ...
    const tryInlineThenDedicatedHeal = (
      ...
      Effect.gen(function* () {
        const inline = yield* maybeInlineHeal(storyAgentId, detail);
        if (inline?.outcome === "no_changes" || inline === undefined) {
          yield* maybeHeal(trigger, detail, verifyOutput, storyAgentId);
          return;
        }
        if (
    ...
    if (options.stack && !options.dryRun) {
      ...
      if (
        ...
        const link = yield* ensureLocalSdkLink(
          frontendCwd,
          sdkCwd,
          config.sdkClientPackage,
        );
        if (link.linked) {
          out.info(
            `${issue.key}: ${config.sdkClientPackage} → ${link.detail}`,
          );
        } else {
    ...
    if (options.postHealResume) {
      yield* store.appendResolutionStep(
        issue.key,
        "Resuming story after issue-dinner self-heal",
      );
    }
    ...
    out.storyHeader(issue.key, issue.summary, "running");
    out.info(formatWorkspacesLabel(roots));
    const local = storyAgentOptions(config, roots, options.toolRoot);
    ...
    const agentResult = yield* withAgent(
      ...
      (agent) =>
        ...
        if (
          ...
          const inlineMsg =
            inlineAfterAgent.error ?? "inline heal failed after story agent edits";
        ...
        if (shouldCommit) {
          commits = yield* commitStoryWithRecovery({
            issue,
            config,
            roots,
            apiKey,
...
const commitStoryWithRecovery = (options: {
  issue: JiraIssue;
  config: MachineConfig;
  roots: IssueWorkspaces;
  apiKey: string;
...
> =>
  Effect.gen(function* () {
    ...
    let results = yield* commitStoryWip(
      options.issue.key,
