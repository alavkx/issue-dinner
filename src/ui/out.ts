...
export function courseHeader(issueKey: string, summary: string, status?: string): void {
...
export function skipCourse(issueKey: string, reason: string): void {
  line(`${stamp()} ${fg.gray("⊘")} ${fg.dim(`${issueKey} — ${reason}`)}`);
}

export function holdCourse(issueKey: string, reason: string): void {
