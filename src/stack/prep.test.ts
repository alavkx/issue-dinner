...
function recordingPort(): GraphiteStackPort & {
  ...
  const port: GraphiteStackPort & {
    ...
    branchExists: (_cwd, branch) =>
      Effect.succeed(branch === "alavoie/cpd-635-user-event-log"),
    currentBranch: () => Effect.succeed("alavoie/cpd-635-user-event-log"),
  ...
  it("tracks an existing stack base before creating story branches", () =>
    runEffect(
      Effect.gen(function* () {
        ...
        assert.ok(track);
        assert.equal(track?.detail, "alavoie/cpd-635-user-event-log<-main");
      }),
    ));
});
