...
import { formatHealStatus, getHealStatus } from "./heal-build.js";
    ...
    withHealRoot(({ root }) =>
      Effect.gen(function* () {
        ...
        try {
          ...
          const appliedDir = join(healAppliedDir(root), "heal-a");
...
describe("contributeAppliedPatches", () => {
  it("opens a PR for an applied patch using the git port", () =>
    withHealRoot(({ root }) =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const appliedDir = join(healAppliedDir(root), "fix-target");
        yield* fs.makeDirectory(appliedDir, { recursive: true });
        yield* fs.writeFileString(
          join(appliedDir, MANIFEST_FILE),
          JSON.stringify({
        ...
        const result = yield* contributeAppliedPatches({ root }).pipe(
          Effect.provide(Layer.succeed(SelfHealGitPort, git)),
        );
...
function mockGitPort(calls: string[]): SelfHealGitCommands {
  return {
    readBranch: () =>
      Effect.sync(() => {
...
function withHealRoot<A, E, R>(
  ...
  return runEffect(
    Effect.gen(function* () {
      ...
      yield* fs.makeDirectory(healAppliedDir(root), { recursive: true });
