...
export function buildHealPrompt(options: {
  ...
  const { issue, kitchenRoot, trigger, detail, verifyOutput, attempt, maxAttempts } =
    options;

  const triggerGuide: Record<HealTriggerKind, string> = {
    orchestration:
      "issue-dinner threw while orchestrating this course — fix the CLI source.",
    agent_error:
      "The course agent failed; if the root cause is issue-dinner (not the project), fix the CLI source.",
    verify:
...
### Context
- **Course blocked:** ${issue.key} — ${issue.summary}
- **Trigger:** ${triggerGuide[trigger]}
- **issue-dinner root (your workspace):** \`${kitchenRoot}\`
...
### How to work
1. Edit \`src/**/*.ts\` under \`${kitchenRoot}\` directly using file tools — **do not** write JSON patch manifests.
...
export function buildPostHealCourseResumePrompt(options: {
  issue: JiraIssue;
  fixSummary: string;
}): string {
  return `## issue-dinner was fixed — continue your course
