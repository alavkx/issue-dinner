...
import {
  ...
  manifestTargetsPackage,
  persistDurableHeal,
  readDurableManifest,
  snapshotSrcFiles,
  syncDurableHealsToPackage,
} from "./durable-patches.js";
import { ISSUE_DINNER_PACKAGE } from "./patch.js";
...
describe("durable-patches", () => {
  ...
  it("skips foreign durable heals when syncing into issue-dinner", () =>
    runEffect(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const pkgRoot = yield* fs.makeTempDirectory();
        ...
        yield* fs.writeFileString(
          ...
          JSON.stringify({ name: ISSUE_DINNER_PACKAGE }),
        );
        yield* fs.writeFileString(join(pkgRoot, "src", "existing.ts"), "old\n");
        ...
        try {
          ...
          yield* persistDurableHeal({
            ...
            files: [
              {
                path: "src/build/vite-plugins.ts",
                content: 'import { x } from "@acme/api-client";\n',
              },
            ],
          });
          ...
          const content = yield* fs.readFileString(join(pkgRoot, "src", "existing.ts"));
          assert.equal(content, "new\n");
          assert.equal(yield* fs.exists(join(pkgRoot, "src", "build")), false);
        } finally {
          if (prevHealDir === undefined) delete process.env.ISSUE_DINNER_STATE_DIR;
          else process.env.ISSUE_DINNER_STATE_DIR = prevHealDir;
        }
  ...
  it("detects foreign manifests", () => {
    assert.equal(
      manifestLooksForeign({
        id: "x",
        files: [{ path: "src/a.ts", content: 'import { x } from "~/components/foo";\n' }],
      }),
      true,
    );
    assert.equal(
      manifestTargetsPackage(
        { id: "x", packageName: "other-app", files: [] },
