...
import { contributeAppliedPatches } from "./contribute.js";
...
const buildReviewPrompt = (
  manifests: ReadonlyArray<KitchenPatchManifest>,
  baseBranch: string,
): string => {
...
export const reviewAndContributeHeals = (options: {
  ...
  HealReviewResult,
  unknown,
  FileSystem.FileSystem | CommandExecutor.CommandExecutor | import("./contribute.js").KitchenGitPort
> =>
  Effect.gen(function* () {
    const pending = yield* listDurableHealManifests();
    if (pending.length === 0) {
      return { approved: [], rejected: [], contributed: [], skipped: true };
    }

    const review = yield* runHealReviewAgent(options);
